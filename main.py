...
# flask-socketio removed — no WebSocket events defined, frontend doesn't use it
import json
import os
import sys
import logging
...
def initialize_system():
    ...
    print("\n" + "="*60)
    print("NEXIRA v12 - INITIALIZATION")
...
def chat():
    try:
        ...
        if style_match and image_gen:
            ...
            if success:
                response_text += f"\n\n[IMG:/api/images/file/{styled_path}]"
...
def search_and_chat():
    ...
    if background_scheduler and background_scheduler.curiosity_engine:
        background_scheduler.curiosity_engine.process_exchange(
            query,
            response_text,
            ollama_model=ai_engine.config.get('ai', {}).get('model', 'llama3.1:8b')
        )
...
def api_health():
    ...
    try:
        ...
        if hasattr(result, 'models'):
            ...
            model_count = 0
        health['systems']['ollama'] = {'status': 'ok', 'detail': f"{model_count} model(s) available"}
    except Exception as e:
    ...
    try:
        ...
        health['systems']['database'] = {'status': 'ok', 'detail': f"{msg_count} messages in history"}
    except Exception as e:
...
def main():
    ...
    # Standard Flask server (no SocketIO needed)
    app.run(
